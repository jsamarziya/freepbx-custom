...
class Callblocker extends \FreePBX_Helpers implements \BMO {
	...
	public function doDialplanHook(&$ext, $engine, $priority) {
		//$ext->addInclude('from-internal-additional', 'callblocker-context'); // Add the context to from from-internal
		$mcontext = 'callblocker-context';
		$ext->add($mcontext, '_X!','', new \ext_noop('Hello from callblocker'));
	}
	...
	public function ucpConfigPage($mode, $user, $action) {
		if(empty($user)) {
			...
			if($mode == 'group') {
				$enabled = $this->FreePBX->Ucp->getSettingByGID($user['id'],'Callblocker','enabled');
				$enabled = !($enabled) ? false : true;
			} else {
				$enabled = $this->FreePBX->Ucp->getSettingByID($user['id'],'Callblocker','enabled');
			}
		}

		$html = array();
		$html[0] = array(
			"title" => _("UCP Module callblocker"),
			"rawname" => "callblocker",
	...
	public function ucpUpdateUser($id, $display, $ucpStatus, $data) {
		if($display == 'userman' && isset($_POST['type']) && $_POST['type'] == 'user') {
			if(isset($_POST['callblocker_enable']) && $_POST['callblocker_enable'] == 'yes') {
				$this->FreePBX->Ucp->setSettingByID($id,'Callblocker','enabled',true);
			}elseif(isset($_POST['callblocker_enable']) && $_POST['callblocker_enable'] == 'no') {
				$this->FreePBX->Ucp->setSettingByID($id,'Callblocker','enabled',false);
			} elseif(isset($_POST['callblocker_enable']) && $_POST['callblocker_enable'] == 'inherit') {
				$this->FreePBX->Ucp->setSettingByID($id,'Callblocker','enabled',null);
			}
		}
	}
	...
	public function ucpUpdateGroup($id,$display,$data) {
		if($display == 'userman' && isset($_POST['type']) && $_POST['type'] == 'group') {
			if(isset($_POST['callblocker_enable']) && $_POST['callblocker_enable'] == 'yes') {
				$this->FreePBX->Ucp->setSettingByGID($id,'Callblocker','enabled',true);
			} else {
				$this->FreePBX->Ucp->setSettingByGID($id,'Callblocker','enabled',false);
			}
		}
	}
